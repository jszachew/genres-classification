...
def train_cd1_logistic_regression(weighted=True):
    ...
    # plot_confusion_matrix(cm=cm, classes=classes, normalize=True)
    print(cm)
    plot_confusion_matrix(cm=cm, classes=classes, title=f"CD1 LogisticRegression sag ovr weighted")
    print(accuracy)
    # print(f"f1: {f1}")
...
def train_cd1_mlp(weighted=True):
    ...
    #print('Mean precision: %.3f' % mean(scores['test_precision_macro']))
    lr.fit(x_train, y_train)
    print(f"accuracy for MLP: {(lr.score(x_test, y_test))}")
...
def train_cd1_SMOTE_random_forest(weighted=True):
    w = {
        "RnB": 2.21383523,
        "Country": 1.63764602,
        "Pop_Rock": 0.00010347397,
    ...
    oversample = SMOTE(n_jobs=-1, k_neighbors=3)
    over_X, over_y = oversample.fit_resample(pd_train_x, pd_train_y)
    x_train, x_test, y_train, y_test = train_test_split(over_X, over_y, test_size=0.2, stratify=over_y)
    print("RandomForestClassifier")

    if weighted:
        lr = RandomForestClassifier(n_estimators=150, verbose=1, random_state=0, n_jobs=-1)
    else:
        lr = RandomForestClassifier(n_estimators=150, verbose=1, n_jobs=-1)

    cv = RepeatedStratifiedKFold(n_splits=10, n_repeats=3, random_state=1)
    scoring = ('f1_macro', 'recall_macro', 'precision_macro')
    # Evaluate BRFC model
    scores = cross_validate(lr, over_X, over_y, scoring=scoring, cv=cv)
    # Get average evaluation metrics
    print('Mean f1: %.3f' % mean(scores['test_f1_macro']))
    print('Mean recall: %.3f' % mean(scores['test_recall_macro']))
    print(scores['test_recall_macro'])
